...
export default function DashboardPage({ navigate }) {
  ...
  const load = useCallback(async (showSpinner = false) => {
    ...
    setPatients(pts)
    setLogs(hist.slice(0, 20))

    const histAll = JSON.parse(localStorage.getItem('pharmatrack_history') || '[]')
    const doneHist = histAll.filter(h => h.is_done === true)
    const byVN = {}
    doneHist.forEach(h => {
      if (!byVN[h.vn] || h.id > byVN[h.vn].id) byVN[h.vn] = h
    })
    setCompleted(Object.values(byVN))
    setLoading(false)
  }, [])
  ...
  const stats = {
    total:          patients.length + completed.length,
