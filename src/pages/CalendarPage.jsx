...
import { getAll, updatePatient, dispensePatient, toISO, fmt, ST } from '../utils/storage'
...
export default function CalendarPage({ navigate, jumpTo }) {
  ...
  const saveDrawer = async ({ curStatus, phone, activeDate }) => {
    ...
    const updates = {
      ...
      phone:            phone || p.phone,
      status:           curStatus,
      activeDate:       activeDate || p.activeDate,
      followupDate:     curStatus === 'production' && activeDate
                          ? (() => { const d = new Date(activeDate + 'T00:00:00'); d.setDate(d.getDate() + 31); return toISO(d) })()
                          : p.followupDate,
      productionDate:   curStatus === 'production' ? activeDate : p.productionDate,
      remainingBottles: p.remainingBottles,
